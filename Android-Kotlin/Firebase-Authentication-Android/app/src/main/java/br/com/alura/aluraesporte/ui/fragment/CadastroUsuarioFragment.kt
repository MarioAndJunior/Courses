...
class CadastroUsuarioFragment : Fragment() {
    ...
    private fun cadastra(usuario: Usuario) {
        viewModel.cadastra(usuario).observe(viewLifecycleOwner, Observer {
            it?.let { cadastrado ->
                view?.let { view ->
                    if (cadastrado.dado) {
                        view.snackBar("Usuário cadastrado com sucesso")
                        controlador.popBackStack()
                    } else {
                        val mensagemErro = cadastrado.erro ?: "Ocorreu uma falha no cadastro"
    ...
    private fun validaCampos(
        ...
        if (email.isBlank()) {
            cadastro_usuario_email.error = "E-mail não pode ser vazio"
            valido = false
        }
